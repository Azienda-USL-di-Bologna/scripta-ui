...
import {
  ...
  colsCSV,
  DocsListMode,
  StatiVersamentoErroriPerFiltro,
  StatiVersamentoParerPerFiltro,
  StatiVersamentoTraduzioneVisualizzazione,
  StatoDocDetailPerFiltro,
  StatoUfficioAttiTraduzioneVisualizzazione,
  TipologiaDocDetailPerFiltro,
  TipologiaDocTraduzioneVisualizzazione,
} from "./docs-list-constants";
...
{
  @Output() showRightPanel = new EventEmitter<{
    showPanel: boolean;
    rowSelected: ExtendedDocDetailView;
    sonoPersonaVedenteSuDocSelezionato: boolean;
  }>();
  @Output() docListModeSelected = new EventEmitter<{
    docListModeSelected: DocsListMode;
  }>();
  @Input() data: any;
  ...
  public DecimalePredicatoVicario: DecimalePredicato =
  ...
  @ViewChild("multiselectStatoUltimoVersamento")
  public multiselectStatoUltimoVersamento: MultiSelect;
  @ViewChild("columnFilterAzienda") public columnFilterAzienda: ColumnFilter;
  @ViewChild("autocompleteIdPersonaRedattrice")
  public autocompleteIdPersonaRedattrice: AutoComplete;
  @ViewChild("autocompleteidPersonaResponsabileProcedimento")
  public autocompleteidPersonaResponsabileProcedimento: AutoComplete;
  @ViewChild("autocompletearchiviDocList")
  public autocompletearchiviDocList: AutoComplete;
  @ViewChild("autocompleteIdStrutturaRegistrazione")
  public autocompleteIdStrutturaRegistrazione: AutoComplete;
  @ViewChild("autocompleteFirmatari")
  public autocompleteFirmatari: AutoComplete;
  @ViewChild("autocompleteSullaScrivaniaDi")
  public autocompleteSullaScrivaniaDi: AutoComplete;
  ...
  public statoUfficioAttiVisualizzazioneObj =
    StatoUfficioAttiTraduzioneVisualizzazione;
  ...
  @Input("showAnteprimaInit") set showAnteprimaInit(
    showAnteprimaInit: boolean
  ) {
    if (showAnteprimaInit != undefined) this.showAnteprima = showAnteprimaInit;
  }
  ...
  ngOnInit(): void {
    this.cols = cols.map((a) => {
      return { ...a };
    });
    this.docsListMode =
      (this.route.snapshot.queryParamMap.get("mode") as DocsListMode) ||
      DocsListMode.MIEI_DOCUMENTI;
      ...
      this.loginService.loggedUser$
        ...
        .subscribe((utenteUtilities: UtenteUtilities) => {
          if (utenteUtilities) {
            ...
            this.isSegretario =
              this.utente.struttureDelSegretario &&
              this.utenteUtilitiesLogin.getUtente().struttureDelSegretario
                .length > 0;
            this.calcDocListModeItem();
            this.selectedButtonItem = this.selectButtonItems.filter(
              (element) => element.queryParams.mode === this.docsListMode
            )[0];
            ...
            if (
              this.utenteUtilitiesLogin.isCA() === false &&
              this.utenteUtilitiesLogin.isCI() === false
            ) {
              this.tipologiaVisualizzazioneObj =
                this.tipologiaVisualizzazioneObj.filter(
                  (item) => item.value !== TipologiaDoc.DOCUMENT_REGISTRO
                );
            }
              ...
              const funzioniItems: MenuItem[] = [
                {
                  ...
                  disabled: false,
                  command: () =>
                    this.openVersamentoMassivoPopup(StatoVersamento.FORZARE),
                },
                {
                  ...
                  disabled: false,
                  command: () =>
                    this.openVersamentoMassivoPopup(
                      StatoVersamento.ERRORE_RITENTABILE
                    ),
                },
              ] as MenuItem[];
            ...
            if (!!!this.archivio) {
              ...
              const bit = this.archivio.permessiEspliciti.find(
                (permessoArchivio: PermessoArchivio) =>
                  permessoArchivio.fk_idPersona.id ===
                  this.utenteUtilitiesLogin.getUtente().idPersona.id
              )?.bit;
              this.loggedUserCanRestoreArchiviation =
                bit >= DecimalePredicato.VICARIO;
              this.loggedUserCanDeleteArchiviation =
                bit >= DecimalePredicato.ELIMINA;
    ...
    this.subscriptions.push(
      this.doclistService.refreshDocs$.subscribe((val: boolean) =>
  ...
  private openVersamentoMassivoPopup(operazione: StatoVersamento) {
    ...
    if (operazione === StatoVersamento.FORZARE) {
      docCoinvolti.push(
        ...this.docsSelected.filter((d) => {
          return (
            d.statoUltimoVersamento == StatoVersamento.ERRORE &&
            (d.versamentoForzabile || d.versamentoForzabileConcordato)
          );
        })
      );
      docImpossibilitati.push(
        ...this.docsSelected.filter((d) => {
          return !(
            d.statoUltimoVersamento == StatoVersamento.ERRORE &&
            (d.versamentoForzabile || d.versamentoForzabileConcordato)
      ...
      docCoinvolti.push(
        ...this.docsSelected.filter((d) => {
          return (
            d.statoUltimoVersamento == StatoVersamento.ERRORE ||
            d.statoUltimoVersamento == StatoVersamento.ERRORE_RITENTABILE
          );
        })
      );
      docImpossibilitati.push(
    ...
    if (docCoinvolti.length === 0) {
      this.messageService.add({
        ...
        key: "docsListToast",
        summary: "Attenzione",
        detail: `Nessuno dei documenti selezionati è ${
          operazione === StatoVersamento.FORZARE ? "forzabile" : "ritentabile"
        }`,
      });
      return;
    }
    ...
    this.versamentoMassivoPopupInfo = {
      title:
        operazione === StatoVersamento.FORZARE
          ? "Forza versamenti"
          : "Ritenta versamenti",
      show: true,
   ...
   */
  private versaMassivamente(
    operazione: StatoVersamento,
    docs: ExtendedDocDetailView[],
    idAzienda: number
  ): void {
    this.docDetailService
      .versaDocMassivo(operazione, docs, idAzienda)
      .subscribe({
        next: (res: any) => {
          this.messageService.add({
            severity: "success",
  ...
  private setStatiVersamentoObj(resetFilter = false) {
    ...
    // Tolgo il filtro
    if (resetFilter) {
      if (this.multiselectStatoUltimoVersamento)
        this.multiselectStatoUltimoVersamento.writeValue([]);
      if (this.dataTable.filters["statoUltimoVersamento"])
        (this.dataTable.filters as any)["statoUltimoVersamento"]["value"] =
          null;
      if (this.dataTable.filters["versamentoForzabile"])
        (this.dataTable.filters as any)["versamentoForzabile"]["value"] = null;
      if (this.dataTable.filters["versamentoForzabileConcordato"])
        (this.dataTable.filters as any)["versamentoForzabileConcordato"][
          "value"
        ] = null;
    }
  }
  ...
  public calcDocListModeItem(): void {
    ...
    if (
      this.utenteUtilitiesLogin.hasRole(CODICI_RUOLO.RV) ||
      this.utenteUtilitiesLogin.hasRole(CODICI_RUOLO.SD)
    ) {
      this.selectButtonItems.push({
        title: "",
  ...
  public onSelectButtonItemSelection(event: any): void {
    ...
    switch (this.docsListMode) {
      ...
      case DocsListMode.REGISTRAZIONI:
        this.calendarcreazione.writeValue(
          this.lastDataCreazioneFilterValue ||
            this.actualDataCreazioneFilterValue
        );
    ...
    if (this.utenteUtilitiesLogin) this.calcolaAziendeFiltrabili();
    this.setStatiVersamentoObj(
      oldDocsListMode === DocsListMode.ERRORI_VERSAMENTO ||
        this.docsListMode === DocsListMode.ERRORI_VERSAMENTO
    );
    this.resetPaginationAndLoadData();
  }
  ...
  set selectedColumns(colsSelected: ColonnaBds[]) {
    if (this._selectedColumns.length > colsSelected.length) {
      this._selectedColumns = this._selectedColumns.filter((sc) =>
        colsSelected.includes(sc)
      );
    } else if (this._selectedColumns.length < colsSelected.length) {
      this._selectedColumns.push(
        colsSelected.find((cs) => !this._selectedColumns.includes(cs))
      );
    }
    if (!this.archivio) {
    ...
    if (!this.isResponsabileVersamento) {
      this.selectableColumns.forEach((value, index) => {
        if (value.field === "dataUltimoVersamento")
          this.selectedColumns.splice(index, 1);
      });
      this.selectedColumns.forEach((value, index) => {
        if (value.field === "dataUltimoVersamento")
          this.selectedColumns.splice(index, 1);
      });
    }
  ...
  public onChangeSelectedColumns(event: any): void {
    if (
      !event.value.some((e: ColonnaBds) => e.field === event.itemValue.field)
    ) {
      // Se itemValue non è dentro l'elenco value allora ho tolto la spunta e quella colonna non è più visibile
      const col = this.filterColumns.find(
        (e: ColumnFilter) => e.field === event.itemValue.filterField
      );
      if (col.hasFilter()) {
        col.clearFilter();
      }
  ...
  private loadConfigurationAndSetItUp(): void {
    this.cols[this.cols.findIndex((c) => c.field === "idArchivi")].header =
      "Fascicolazioni";
    ...
    this.selectableColumns = cols
      .filter((c) => !colonneDaNonVisualizzare.includes(c.field))
      .map((e) => {
        if (this.mandatoryColumns.includes(e.field)) {
          e.selectionDisabled = true;
        }
        if (
          !this.isResponsabileVersamento &&
          e.field === "dataUltimoVersamento"
        ) {
          e.selectionDisabled = true;
        }
        return e;
    ...
    const impostazioni =
      this.utenteUtilitiesLogin.getImpostazioniApplicazione();
    if (
      impostazioni &&
      impostazioni.impostazioniVisualizzazione &&
      impostazioni.impostazioniVisualizzazione !== ""
    ) {
      const settings: Impostazioni = JSON.parse(
        impostazioni.impostazioniVisualizzazione
      ) as Impostazioni;
  ...
  public setColumnsPerDetailArchivio(): void {
    ...
    this.cols[this.cols.findIndex((c) => c.field === "idArchivi")].header =
      "Altre fascicolazioni"; // Modifica custom all'header delle fascicolazioni.
  ...
  public saveConfiguration() {
    const impostazioniVisualizzazione =
      this.utenteUtilitiesLogin.getImpostazioniApplicazione()
        ? this.utenteUtilitiesLogin.getImpostazioniApplicazione()
            .impostazioniVisualizzazione
        : null;
    let impostazioniVisualizzazioneObj: Impostazioni;
    if (impostazioniVisualizzazione && impostazioniVisualizzazione !== "") {
      impostazioniVisualizzazioneObj = JSON.parse(
        impostazioniVisualizzazione
      ) as Impostazioni;
      if (!impostazioniVisualizzazioneObj["scripta.docList"]) {
        impostazioniVisualizzazioneObj["scripta.docList"] =
          {} as ImpostazioniDocList;
      }
    } else {
    ...
    impostazioniVisualizzazioneObj["scripta.docList"].mieiDocumenti =
      this.mieiDocumenti;
  ...
  private calcolaAziendeFiltrabili() {
    ...
    if (
      this.docsListMode !== DocsListMode.REGISTRAZIONI ||
      this.utenteUtilitiesLogin.hasRole(CODICI_RUOLO.SD)
    ) {
      this.aziendeFiltrabili = this.utenteUtilitiesLogin
        .getUtente()
        .aziendeAttive.map((a) => {
      ...
      */
      ...
      if (
        this.utenteUtilitiesLogin.getUtente().ruoliUtentiPersona[
          CODICI_RUOLO.OS
        ]
      ) {
        codiceAziendeOSMOS = codiceAziendeOSMOS.concat(
          this.utenteUtilitiesLogin.getUtente().ruoliUtentiPersona[
            CODICI_RUOLO.OS
          ]["GENERALE"]
        );
      ...
      this.aziendeFiltrabili = this.utenteUtilitiesLogin
        .getUtente()
        .aziendeAttive.filter((a) => codiceAziendeOSMOS.indexOf(a.codice) != -1)
        .map((a) => {
          aziendeUtente.push(a.id);
          return { value: [a.id], label: a.nome } as ValueAndLabelObj;
        });
    ...
    if (this.aziendeFiltrabili) {
      this.subscriptions.push(
        this.configurazioneService
          .getParametriAziende("dataInizioPregressi", null, aziendeUtente)
          .subscribe((data: ParametroAziende[]) => {
  ...
  private buildCustomFilterAndSort(): FiltersAndSorts {
    ...
    if (!!this.archivio) {
      ...
      filterAndSort.addAdditionalData(
      ...
      filterAndSort.addAdditionalData(
        new AdditionalDataDefinition("idArchivio", this.archivio.id.toString())
      );
    }
    switch (this.docsListMode) {
      case DocsListMode.DOCUMENTI_VISIBILI:
  ...
  private loadData(idDocListToSelect?: number[]): void {
    ...
    if (this.storedLazyLoadEvent.sortField === "dataRegistrazione") {
      lazyFiltersAndSorts.addSort(
        new SortDefinition(
          "dataCreazione",
          this.storedLazyLoadEvent.sortOrder === 1
            ? SORT_MODES.asc
            : SORT_MODES.desc
        )
      );
    ...
    this.loadCount(
      this.serviceForGetData,
      this.projectionFotGetData,
      filtersAndSorts,
      lazyFiltersAndSorts
    );
    this.loadDocsListSubscription = this.serviceForGetData
      .getData(
        this.projectionFotGetData,
        filtersAndSorts,
        lazyFiltersAndSorts,
      ...
      .subscribe({
        next: (data: any) => {
          ...
          if (
            this.pageConf.conf.offset === 0 &&
            data.page.totalElements < this.pageConf.conf.limit
          ) {
            ...
            Array.prototype.splice.apply(this.docs, [
              0,
              this.docs.length,
              ...this.setCustomProperties(data.results),
            ]);
          } else {
            Array.prototype.splice.apply(this.docs, [
  ...
  private setCustomProperties(
    docsList: DocDetailView[]
  ): ExtendedDocDetailView[] {
    const extendedDocsList: ExtendedDocDetailView[] =
      docsList as ExtendedDocDetailView[];
    extendedDocsList.forEach((doc: ExtendedDocDetailView) => {
      Object.setPrototypeOf(doc, ExtendedDocDetailView.prototype);
      if (this.archivio) {
        doc.archiviation = doc.archiviDocList?.find(
          (archivioDoc) => archivioDoc.idArchivio.id === this.archivio.id
        );
      }
  ...
  public filterPersone(event: any) {
    ...
    filtersAndSorts.addFilter(
      new FilterDefinition(
        "descrizione",
        FILTER_TYPES.string.startsWithIgnoreCase,
        event.query
      )
    );
    ...
    this.personaService
      .getData(null, filtersAndSorts, null)
      .subscribe((res) => {
        if (res && res.results) {
          res.results.forEach((persona: any) => {
            persona["descrizioneVisualizzazione"] =
              persona.descrizione +
              (persona.idSecondario ? " (" + persona.idSecondario + ")" : "");
          });
  ...
  public filterArchivi(event: any) {
    ...
    filtersAndSorts.addFilter(
      new FilterDefinition("tscol", FILTER_TYPES.not_string.equals, event.query)
    );
    ...
    filtersAndSorts.addAdditionalData(
      new AdditionalDataDefinition(
        "BitPermessoMinimo",
        DecimalePredicato.VISUALIZZA.toString()
      )
    );
    this.archivioDetailViewService
      .getData(null, filtersAndSorts, null)
      .subscribe((res) => {
        if (res && res.results) {
          res.results.forEach((archivio: any) => {
            archivio["descrizioneVisualizzazione"] =
              "[" + archivio.numerazioneGerarchica + "] " + archivio.oggetto;
          });
  ...
  public buildJsonValueForFilterPersone(idPersona: number): string {
    if (idPersona) {
      const filtroJson: FilterJsonDefinition<PersonaUsante> =
        new FilterJsonDefinition(true);
  ...
  public filterStrutture(event: any) {
    ...
    (this.dataTable.filters["idAzienda.id"] as any).value.forEach(
      (idAzienda: number) => {
        filtersAndSorts.addFilter(
          new FilterDefinition(
            "idAzienda.id",
    ...
    filtersAndSorts.addFilter(
      new FilterDefinition("ufficio", FILTER_TYPES.not_string.equals, false)
    );
    ...
    this.strutturaService
      .getData("StrutturaWithIdAzienda", filtersAndSorts, null)
      .subscribe((res) => {
        if (res && res.results) {
  ...
  public clear(): void {
    ...
    if (this.autocompleteIdPersonaRedattrice)
      this.autocompleteIdPersonaRedattrice.writeValue(null);
    if (this.autocompleteidPersonaResponsabileProcedimento)
      this.autocompleteidPersonaResponsabileProcedimento.writeValue(null);
    if (this.autocompletearchiviDocList)
      this.autocompletearchiviDocList.writeValue(null);
    if (this.autocompleteIdStrutturaRegistrazione)
      this.autocompleteIdStrutturaRegistrazione.writeValue(null);
    if (this.autocompleteFirmatari) this.autocompleteFirmatari.writeValue(null);
    if (this.autocompleteSullaScrivaniaDi)
      this.autocompleteSullaScrivaniaDi.writeValue(null);
      ...
      .subscribe(
        (res) => {
          if (res && res.results) {
            ...
            tableTemp.columns = colsCSV.filter((c) =>
              this.selectedColumns.some((e) => e.field === c.fieldId)
            );
  ...
  public isEliminabile(doc: ExtendedDocDetailView): boolean {
    ...
    return doc.sullaScrivaniaDi.some(
      (p) => p.idPersona === this.utenteUtilitiesLogin.getUtente().idPersona.id
    );
  }
   ...
   */
  public confermaEliminaProposta(
    doc: ExtendedDocDetailView,
    event: Event
  ): void {
    this.confirmationService.confirm({
      ...
      target: event.target,
      message:
        "Stai eliminando questa proposta e i suoi eventuali allegati, vuoi proseguire?",
      accept: () => {
        this.loading = true;
   ...
   * @param event
   */
  public handleCalendarButtonEvent(
    calendar: Calendar,
    command: string,
    event: Event,
    filterCallback: (value: Date[]) => {}
  ) {
    if (command === "doFilter" || command === "onClickOutside") {
      // pulsante OK

      calendar.hideOverlay();
    } else if (command === "setToday") {
      // pulsante OGGI
   ...
   * @param event
   */
  public askConfirmAndHandleCalendarCreazioneEvent(
    calendar: Calendar,
    command: string,
    event: Event,
    filterCallback: (value: Date[]) => {}
    ...
    if (command === "onClickOutside") {
      if (this.docsListMode === DocsListMode.PREGRESSI) {
        return;
      }
    ...
    if (
      command === "clear" ||
      !!!calendar.value ||
      calendar.value[0] === null
    ) {
      // Sto cercando su tutti gli anni
      this.needToAsk = true;
    } else {
      if (
        calendar.value[1] !== null &&
        calendar.value[1].getYear() - calendar.value[0].getYear() > 1
      ) {
      ...
      setTimeout(() => {
        this.confirmationService.confirm({
          ...
          accept: () => {
            ...
            this.handleCalendarButtonEvent(
              calendar,
              command,
              event,
              filterCallback
   ...
   */
  public filterAzienda(
    filterCallback: (value: number[]) => {},
    value: number[],
    filteraziendacontainer: any
  ) {
    if (value.length === 1) {
   ...
   * @param event
   */
  public filterStatoUltimoVersamento(
    ...
    if (
      itemValue === "Errore forzabile" ||
      itemValue === "Errore non forzabile"
    ) {
      if (value.length > 0) {
      ...
      this.multiselectStatoUltimoVersamento.writeValue(
        value.filter((v: string) => v !== "Errore forzabile")
      );
      let array: string[] = [];
      value.forEach((labelStato: string) => {
  ...
  public filterStatiVersamento(event: any) {
    ...
    for (let i = 0; i < this.statiVersamentoObj.length; i++) {
      ...
      if (
        statoVersamento.nome.toLowerCase().indexOf(query.toLowerCase()) == 0
      ) {
  ...
  public restoreArchiviation(doc: ExtendedDocDetailView): void {
    ...
    this.subscriptions.push(
      this.archivioDocService
        .patchHttpCall(archivioDocToUpdate, archivioDocToUpdate.id)
        .subscribe((res) => {
   ...
   * @param doc
   */
  public deleteArchiviation(
    doc: ExtendedDocDetailView,
    iconaFunzioniArchiviazione: any
  ): void {
    if (
      [
        ...
        "DETERMINA",
        "DELIBERA",
      ].includes(doc.tipologia) &&
      !doc.archiviDocList.some(
        (archivioDoc) =>
          !archivioDoc.dataEliminazione &&
          archivioDoc.id !== doc.archiviation.id
      )
    ) {
      ...
      this.confirmationService.confirm({
        ...
        message: `Stai per eliminare logicamente la fascicolazione di ${
          doc.registrazioneVisualizzazione
            ? doc.registrazioneVisualizzazione
            : doc.oggettoVisualizzazione
        }. Vuoi procedere?`,
        icon: "pi pi-exclamation-triangle",
        accept: () => {
          // L'utente conferma di voler cercare su tutte le sue aziende. faccio quindi partire il filtro
          ...
          archivioDocToUpdate.idPersonaEliminazione = {
          ...
          this.subscriptions.push(
            this.archivioDocService
              ...
              .subscribe((res) => {
                doc.archiviation.dataEliminazione = res.dataEliminazione;
                doc.archiviation.idPersonaEliminazione =
                  res.idPersonaEliminazione;
                doc.archiviation.version = res.version;
                this.messageService.add({
                  severity: "success",
                  key: "docsListToast",
   ...
   */
  public openArchive(
    archivioDoc: ArchivioDoc,
    doc?: ExtendedDocDetailView
  ): void {
    this.navigationTabsService.addTabArchivio(archivioDoc.idArchivio);
    this.appService.appNameSelection(
      "Fascicolo " +
        archivioDoc.idArchivio.numerazioneGerarchica +
        " [" +
        archivioDoc.idArchivio.idAzienda.aoo +
        "]"
    );
  }
  ...
  public openDetailAndPreview(doc: ExtendedDocDetailView): void {
    ...
    this.personaVedenteService
      .getData("PersonaVedenteWithPlainFields", filtersAndSorts, null)
      .subscribe((res) => {
        if (res && res.results) {
      ...
      .add(() => {
        ...
        this.showRightPanel.emit({
          showPanel: true,
          rowSelected: doc,
          sonoPersonaVedenteSuDocSelezionato:
  ...
  public onRowSelect(event: any): void {
    ...
    if (this.archivio) this.showAnteprima = true;
  ...
  public setVersamentoVisto(event: any, doc: ExtendedDocDetailView): void {
    ...
    this.docDetailService
      .patchHttpCall(docDetail, docDetail.id)
      .subscribe((res) => {
    ...
    const funzioniItems: MenuItem[] = [
      {
        ...
        disabled: this.isArchivioChiuso(),
      },
      {
        label: "Elimina",
        command: () => {
          this.preDeleteArchiviation();
        },
  ...
  public preDeleteArchiviation() {
    setTimeout(() => {
      this.deleteArchiviation(
        this.docSelezionatoToFunctions,
        this.iconaFunzioniArchiviazioneSelezionatoToFunctions
      );
    }, 0);
  }

  public onClickOnFunctionButton(
    event: Event,
    rowData: ExtendedDocDetailView,
    iconaFunzioniArchiviazione: any
  ) {
    ...
    switch (this.operazioneOrganizza) {
      case "Sposta":
        this.extendedArchivioService
          .spostaDoc(
            this.docSelezionatoToFunctions.id,
            this.archivio.id,
