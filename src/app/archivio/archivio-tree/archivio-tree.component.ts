...
export class ArchivioTreeComponent implements OnInit {
  ...
  get archivio(): Archivio | ArchivioDetail {
    ...
    this.subscriptions.push(
      this.archivioUtilsService.updateArchivioFieldEvent.subscribe(
        (archivioFieldUpdating: ArchivioFieldUpdating) => {
          if (
            archivioFieldUpdating.field === "oggetto" &&
            archivioFieldUpdating.archivio.fk_idArchivioRadice.id ===
              this.archivio.fk_idArchivioRadice.id
          ) {
  ...
  private loadConfig(): void {
    ...
    this.configurationService
      .getParametriAziende("numeroMaxSottoarchiviCaricabili", null, [
        this.archivio.fk_idAzienda.id,
      ])
      .subscribe((parametriAziende: ParametroAziende[]) => {
        if (parametriAziende && parametriAziende[0]) {
          this.numeroMaxSottoarchiviCaricabili = parametriAziende[0]
            .valore as number;
          this.firstTreeLoad();
        }
      });
  ...
  private firstTreeLoad(): void {
    ...
    switch (this.archivio.livello) {
      case 1:
        ...
        if (
          archivioRadice.numeroSottoarchivi <
          this.numeroMaxSottoarchiviCaricabili
        ) {
          ...
          this.serviceToGetData
            ...
            .subscribe((res) => {
              ...
              this.archivi.push(
                this.buildTreeNode(this.archivio, childrenNodes)
              );
            });
        } else {
      ...
      case 2:
        this.getArchivioById(this.archivio.fk_idArchivioPadre.id).subscribe(
          (archivioPa) => {
            ...
            if (
              archivioPadre &&
              archivioPadre.numeroSottoarchivi <
                this.numeroMaxSottoarchiviCaricabili
            ) {
              const filtersAndSorts =
              ...
              this.serviceToGetData
                ...
                .subscribe((res) => {
                  ...
                  this.archivi.push(
                    this.buildTreeNode(archivioPadre, brothersNodes)
                  );
                });
            } else {
              this.troppiSottoFascicoli = true;
      ...
      case 3:
        ...
        ]).subscribe(([archivioPadre, archivioNonno]) => {
          if (
            archivioNonno.numeroSottoarchivi <=
              this.numeroMaxSottoarchiviCaricabili &&
            archivioPadre.numeroSottoarchivi <=
              this.numeroMaxSottoarchiviCaricabili
          ) {
                    ...
                    this.serviceToGetData
                      ...
                      .subscribe((res) => {
                        ...
                        nonnoNodo.children.push(
                          this.buildTreeNode(archivioPadre, childrenNodes)
                        );
                      });
                  }
                }
  ...
  private addTreeNode(archivio: Archivio | ArchivioDetail) {
    ...
    switch (archivio.livello) {
      case 1:
        ...
        break;
      case 2:
        const index = this.archivi[0].children.findIndex(
          (nodo) => (nodo.data as Archivio).id === archivio.id
        );
        if (index === -1) {
          // Il nodo non esiste, lo devo inserire ex novo
          const indexArchivioSuccessivo = this.archivi[0].children.findIndex(
            (nodo) => (nodo.data as Archivio).numero > archivio.numero
          );
          if (indexArchivioSuccessivo === -1) {
            this.archivi[0].children.push(nodoBuildato);
          } else {
            this.archivi[0].children.splice(
              indexArchivioSuccessivo,
              0,
              nodoBuildato
            );
          }
        ...
        break;
      case 3:
        const indexLiv2 = this.archivi[0].children.findIndex(
          (nodo) =>
            (nodo.data as Archivio).id === archivio.fk_idArchivioPadre.id
        );
        if (indexLiv2 !== -1) {
          // Il sottofascicolo padre esiste già. Devo solo aggiungere l'inserto se non c'è già.
          const indexLiv3 = this.archivi[0].children[
            indexLiv2
          ].children.findIndex(
            (nodo) => (nodo.data as Archivio).id === archivio.id
          );
          if (indexLiv3 === -1) {
            const indexArchivioSuccessivo = this.archivi[0].children[
              indexLiv2
            ].children.findIndex(
              (nodo) => (nodo.data as Archivio).numero > archivio.numero
            );
            if (indexArchivioSuccessivo === -1) {
              ...
              this.archivi[0].children[indexLiv2].children.splice(
                indexArchivioSuccessivo,
                0,
                nodoBuildato
              );
            ...
            this.archivi[0].children[indexLiv2].children[indexLiv3] =
              nodoBuildato;
            this.selectedNode =
              this.archivi[0].children[indexLiv2].children[indexLiv3];
          }
          ...
          this.getArchivioById(archivio.fk_idArchivioPadre.id).subscribe(
            (archivioPadre) => {
              const newNode: TreeNode = this.buildTreeNode(archivioPadre, [
                nodoBuildato,
              ]);
              const indexArchivioSuccessivo =
                this.archivi[0].children.findIndex(
                  (nodo) =>
                    (nodo.data as Archivio).numero > archivioPadre.numero
                );
              if (indexArchivioSuccessivo === -1) {
      ...
      this.serviceToGetData
        ...
        .subscribe((res) => {
          ...
          for (let i = 0; i < archiviFigli.length; i++) {
            if (
              !this.archivi[0].children[indexLiv2].children.find(
                (nodo) => (nodo.data as Archivio).id === archiviFigli[i].id
              )
            ) {
              this.archivi[0].children[indexLiv2].children.push(
                this.buildTreeNode(archiviFigli[i])
              );
            }
          }
        });
  ...
  private buildFilterToLoadChildren(
    ...
    //non vogliamo che gli utenti che possono vederli per il loro ruolo non possano vederli
    if (
      !this.utenteUtilitiesLogin.isCA() &&
      !this.utenteUtilitiesLogin.isSD() &&
      !this.utenteUtilitiesLogin.isOS() &&
  ...
  private getArchivioById(idArchivio: number): Observable<Archivio> {
    return this.archivioService.getByIdHttpCall(
      idArchivio,
  ...
  public onNodeSelect(event: any): void {
    ...
    this.appService.appNameSelection(
      "Fascicolo " +
        event.node.data.numerazioneGerarchica +
        " [" +
        event.node.data.idAzienda.aoo +
        "]"
    );
  ...
  private deleteNode(
    alberatura: TreeNode[],
    idArchivioDelNodoDaEliminare: number
  ): boolean {
    const indexNodoDaEliminare = alberatura.findIndex(
      (a) => a.key === idArchivioDelNodoDaEliminare.toString()
    );
    ...
    for (const subAlberatura of alberatura) {
      if (subAlberatura.children) {
        const eliminato = this.deleteNode(
          subAlberatura.children,
          idArchivioDelNodoDaEliminare
        );
        if (eliminato) {
          return true;
        }
