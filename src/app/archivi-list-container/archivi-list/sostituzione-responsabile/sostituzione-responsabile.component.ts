...
export class SostituzioneResponsabileComponent {
  ...
  ngOnInit(): void {
    // const aziende: number[] = [];
    // aziende.push(this.idAzienda);
   ...
   */
  public onSelectedNewResponsabileGestioneMassiva(
    ...
    this.subscriptions.push(
      this.loadStruttureOfUtente(
        utenteStruttura.idUtente,
        this.idAzienda
      ).subscribe((data: any) => {
        if (data && data.results) {
          const utentiStruttura: UtenteStruttura[] = <UtenteStruttura[]>(
            data.results
          );
  ...
  public confermaAvvioSostituzioneResponsabileMassiva(event: Event) {
    if (
      this.idStrutturaFiltro !== this.strutturaUtenteSelectedGestioneMassiva.id
    ) {
